...
# Deep inject core Streamlit UI frame removals
st.markdown("""
    <style>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .screen-login {
            ...
            background: linear-gradient(135deg, rgba(6,2,11,0.6), rgba(20,5,25,0.85)), url("__LOGIN_BG_URL__") no-repeat center center;
            background-size: cover; display: flex; align-items: center; justify-content: center;
            z-index: 10; transition: all 1.2s cubic-bezier(0.4, 0, 0.2, 1); padding: 20px;
        }
        .neon-login-box {
        ...
        .neon-login-box h2 {
            font-family: 'Great Vibes', cursive; font-size: 45px; color: #ff6b9d;
            margin-bottom: 12px; text-shadow: 0 0 15px rgba(255,107,157,0.6);
        }
        .neon-login-box p { font-size: 14px; color: #dcd1e5; margin-bottom: 30px; font-family: 'Montserrat', sans-serif; }
        ...
        .input-wrapper i { position: absolute; left: 20px; top: 50%; transform: translateY(-50%); color: #ff6b9d; font-size: 18px; }
        
        .neon-btn {
            width: 100%; padding: 15px; background: linear-gradient(45deg, #ff6b9d, #ff85a2);
            border: none; border-radius: 50px; color: white; font-size: 16px; font-weight: bold;
        ...
        .glass-photo-frame {
            ...
            padding: 20px; border-radius: 24px; max-width: 450px; width: 100%; text-align: center;
        ...
        .frame-img { width: 100%; max-height: 40vh; object-fit: cover; border-radius: 16px; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.2); }
        .glass-photo-frame h3 { font-family: 'Great Vibes', cursive; font-size: 38px; color: #ff6b9d; margin-bottom: 10px; }
        .glass-photo-frame p { font-family: 'Montserrat', sans-serif; font-size: 14px; color: #e2d9eb; margin-bottom: 20px; line-height: 1.6; }
        ...
        .curtain-trigger {
            ...
            background: rgba(6, 2, 11, 0.9); border: 2px solid #ff6b9d; padding: 18px 35px; border-radius: 50px;
            text-align:center; cursor:pointer; box-shadow: 0 0 30px rgba(255,107,157,0.4); transition: all 0.4s;
        }
        .curtain-trigger h4 { font-size: 18px; color: #ff6b9d; letter-spacing: 1px; margin-bottom: 4px; }
        .curtain-trigger p { font-size: 12px; color: #aaa; font-family: 'Montserrat', sans-serif; }
        ...
        .screen-puzzle { position: absolute; width:100%; height:100%; display:none; background:#06020b; align-items:center; justify-content:center; padding:20px; }
        .puzzle-box { max-width: 440px; width: 100%; text-align: center; }
        .puzzle-box h2 { font-family: 'Great Vibes', cursive; font-size: 40px; color: #ff6b9d; margin-bottom: 8px; }
        .puzzle-box p { font-size: 13px; color: #aaa; margin-bottom: 20px; font-family: 'Montserrat', sans-serif; }
        
        .grid-container {
            display: grid; grid-template-columns: repeat(3, var(--tile-dim, 90px)); grid-template-rows: repeat(3, var(--tile-dim, 90px));
            gap: 6px; justify-content: center; background: rgba(255,255,255,0.03); padding: 8px; border-radius: 16px;
            border: 1px solid rgba(255,255,255,0.1); margin: 0 auto 25px; width: fit-content;
        }
        .tile {
            width: var(--tile-dim, 90px); height: var(--tile-dim, 90px); border-radius: 8px; cursor: pointer;
            background-image: url("__PUZZLE_IMG_URL__"); background-size: calc(var(--tile-dim, 90px) * 3) calc(var(--tile-dim, 90px) * 3);
        ...
        .tile-hint-num { position: absolute; bottom: 4px; right: 4px; font-size: 10px; background: rgba(0,0,0,0.6); padding: 1px 4px; border-radius: 3px; font-weight: bold; color: #fff; }
        ...
        .album-layout {
            max-width: 900px; width: 100%; display: flex; flex-direction: column; gap: 30px; align-items: center; margin: auto;
        }
        ...
        .hero-profile-card {
            background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255,255,255,0.1);
            backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
            border-radius: 30px; padding: 35px; text-align: center; width: 100%;
        ...
        .circle-avatar {
            ...
            background: linear-gradient(45deg, #ff6b9d, #bb6bff); margin-bottom: 20px;
            box-shadow: 0 10px 25px rgba(255,107,157,0.4);
        }
        .circle-avatar img { width:100%; height:100%; object-fit:cover; border-radius:50%; border: 3px solid #06020b; }
        .hero-profile-card h1 { font-family: 'Great Vibes', cursive; font-size: 46px; color: #ff6b9d; margin-bottom: 5px; }
        .hero-profile-card .subtitle { font-size: 18px; color: #bb6bff; margin-bottom: 15px; font-weight: bold; letter-spacing: 1px; }
        .hero-profile-card p.wishes { font-family: 'Montserrat', sans-serif; font-size: 15px; line-height: 1.8; color: #e2d9eb; max-width: 650px; }
        ...
        .flip-back {
            ...
            transform: rotateY(180deg); padding: 20px; border: 1px solid rgba(255, 107, 157, 0.3);
        }
        .flip-back h4 { font-size: 18px; color: #ff6b9d; margin-bottom: 8px; }
        .flip-back p { font-family: 'Montserrat', sans-serif; font-size: 12px; color: #dcd1e5; line-height: 1.5; }
        ...
        .music-bar {
            ...
            border: 1px solid rgba(255,107,157,0.3); padding: 10px 16px; border-radius: 50px;
            display: flex; align-items: center; gap: 10px; box-shadow: 0 8px 25px rgba(0,0,0,0.5);
        }
        .music-bar span { font-size: 11px; font-weight: bold; color: #e2d9eb; }
        .wave-container { display: flex; align-items: flex-end; gap: 2px; height: 13px; width: 18px; }
        ...
        @media(max-width: 600px) {
            ...
            .hero-profile-card h1 { font-size: 34px; }
            .neon-login-box { padding: 30px 20px; }
        }
    </style>
</head>
<body>
    ...
    <script>
        ...
        function renderMatrix() {
            ...
            if(!board) return;
            board.innerHTML = '';
            const dim = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--tile-dim')) || 90;
        ...
        function generateBlossoms() {
            ...
            for (let i = 0; i < 20; i++) {
                setTimeout(() => {
                    ...
                    leaf.style.fontSize = Math.random() * 16 + 15 + "px";
...
components.html(compiled_cinematic_experience, height=720, scrolling=False)
